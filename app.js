...
function renderFullReading(p) {
  ...
  const block = (label, body) => `<p class="sc-block-label fr-h">${label}</p>${body}`;
  return `<div class="fr-doc">` +
    block("はじめに", `<p class="fr-p">この鑑定書は、あなたの名前・生年月日・出生時間・出生地を入り口に、古代の叡智と最新の分析学を重ねて読み解いた、あなただけの記録です。あなたのソウルコードは <b>${p.soulCode}</b>。この並びを持つ人は、世界にあなたしか存在しません。</p>`) +
    block("あなたを動かす5つの力", coresHtml) +
    `<div class="fr-mission"><p class="sc-block-label">あなたの魂の使命</p><p class="fr-mission-jp">${p.soulMission || ""}</p><p class="fr-mission-en">${p.soulMissionEn || ""}</p></div>` +
    block("才能と活かし方", `<p class="fr-p">あなたの強み（${CORE_WORD[top1]} × ${CORE_WORD[top2]}）が最も生きるのは、<b>${pr.jobs.join("・")}</b>といった分野です。「${CORE_WORD[top1]}」を軸に動き、「${CORE_WORD[top2]}」で形にしていくと、あなたらしい成果が生まれます。</p>`) +
    block("人間関係", `<p class="fr-p">自分らしさを尊重し合える相手と、心地よい関係を築けます。あなたの「${CORE_WORD[top1]}」はまわりに良い影響を広げ、信頼を呼びます。早さや深さの違う人には、少し歩幅を合わせると、より深くつながれます。</p>`) +
    block("これからの過ごし方", `<p class="fr-p">完璧を待たず、いちばん自由でいられる選択を。あなたが一歩動くたびに道はできていきます。その歩みは、いつか誰かの希望になります。</p>`) +
    block("実用ガイド", `<div class="fr-mini">🎨 ラッキーカラー：${pr.color}　／　💎 ${pr.stone}<br>💼 向いている分野：${pr.jobs.join("・")}　／　📅 ${pr.day}</div>`) +
